      ...
      if (bailId) {
        ...
        if (bail) {
          setForm({
            ...
            nombre_pieces: bail.nombre_pieces?.toString() || '', etage: bail.etage || '',
            equipements: bail.equipements || '', classe_dpe: bail.classe_dpe || 'D', numero_lot: bail.numero_lot || '',
            ...
            date_exigibilite: bail.date_exigibilite?.toString() || '1', revision_irl: bail.revision_irl ?? true,
            date_debut: bail.date_debut || '', date_fin: bail.date_fin || '',
            clauses: bail.clauses || '', relance_auto_active: bail.relance_auto_active || false,
...
async function envoyerVersYousign() {
  ...
  async function finaliserEtSauvegarder() {
    ...
    try {
      ...
      ligne('Date de début :', form.date_debut ? new Date(form.date_debut).toLocaleDateString('fr-FR') : null)
      ligne('Date de fin :', form.date_fin ? new Date(form.date_fin).toLocaleDateString('fr-FR') : 'Reconduction tacite (1 an)')
      // DIFFÉRENCE MEUBLÉ : durée 1 an
      ...
      // DIFFÉRENCE MEUBLÉ : dépôt 2 mois max
      ligne('Dépôt de garantie :', form.depot_garantie ? `${form.depot_garantie} € (2 mois max pour meublé)` : null)
      ...
      texte(`Un dépôt de garantie de ${form.depot_garantie || 0} € est versé à la signature. Pour un bail meublé, ce dépôt ne peut excéder 2 mois de loyer hors charges. Il sera restitué dans un délai d'un mois si l'état des lieux de sortie est conforme, ou de deux mois dans le cas contraire.`)
    ...
    <main style={{ minHeight: '100vh', background: '#f9fafb', padding: '40px 16px' }}>
      <div style={{ maxWidth: 680, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          <span style={{ background: '#f5f3ff', color: '#7c3aed', padding: '3px 10px', borderRadius: 99, fontSize: 12, fontWeight: 600 }}>🛋️ Meublé</span>
        </div>
        <h1 style={{ fontSize: 26, fontWeight: 700, color: '#111827', marginBottom: 4 }}>Nouveau bail meublé</h1>
        <p style={{ color: '#6b7280', fontSize: 14, marginBottom: 28 }}>Loi n°89-462 — Durée minimale 1 an — Reconduction tacite</p>
          ...
          {etapes.map((e, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', flex: i < etapes.length - 1 ? 1 : 0 }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <div style={{ width: 32, height: 32, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13, background: etape === i+1 ? '#7c3aed' : etape > i+1 ? '#16a34a' : '#e5e7eb', color: etape >= i+1 ? 'white' : '#9ca3af' }}>
                  {etape > i+1 ? '✓' : i+1}
                </div>
                <span style={{ fontSize: 11, color: etape === i+1 ? '#7c3aed' : '#9ca3af', fontWeight: 500, whiteSpace: 'nowrap' }}>{e}</span>
              </div>
        ...
        <div style={{ background: 'white', borderRadius: 16, padding: 28, boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
            ...
            <div>
              ...
              <div style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button type="button" onClick={() => setForm({...form, bailleur_type: 'particulier'})} style={{ flex: 1, padding: 10, borderRadius: 10, border: `2px solid ${form.bailleur_type === 'particulier' ? '#7c3aed' : '#e5e7eb'}`, background: form.bailleur_type === 'particulier' ? '#f5f3ff' : 'white', color: form.bailleur_type === 'particulier' ? '#7c3aed' : '#6b7280', fontWeight: 600, cursor: 'pointer', fontSize: 14 }}>👤 Particulier</button>
                  <button type="button" onClick={() => setForm({...form, bailleur_type: 'morale'})} style={{ flex: 1, padding: 10, borderRadius: 10, border: `2px solid ${form.bailleur_type === 'morale' ? '#7c3aed' : '#e5e7eb'}`, background: form.bailleur_type === 'morale' ? '#f5f3ff' : 'white', color: form.bailleur_type === 'morale' ? '#7c3aed' : '#6b7280', fontWeight: 600, cursor: 'pointer', fontSize: 14 }}>🏢 Société (SCI, SARL...)</button>
                </div>
              </div>
              ...
              <button onClick={() => { if (form.bailleur_type === 'morale') { if (!form.bailleur_denomination || !form.bailleur_adresse) { alert('Dénomination sociale et adresse obligatoires.'); return } } else { if (!form.bailleur_prenom || !form.bailleur_nom || !form.bailleur_adresse) { alert('Prénom, nom et adresse obligatoires.'); return } } setEtape(2) }}
                style={{ width: '100%', background: '#7c3aed', color: 'white', padding: 12, borderRadius: 10, border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: 15 }}>
                Suivant → Locataire
              </button>
            </div>
          )}
          ...
          {etape === 2 && (
            <div>
              ...
              <div style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button type="button" onClick={() => setForm({...form, locataire_type: 'particulier'})} style={{ flex: 1, padding: 10, borderRadius: 10, border: `2px solid ${form.locataire_type === 'particulier' ? '#7c3aed' : '#e5e7eb'}`, background: form.locataire_type === 'particulier' ? '#f5f3ff' : 'white', color: form.locataire_type === 'particulier' ? '#7c3aed' : '#6b7280', fontWeight: 600, cursor: 'pointer', fontSize: 14 }}>👤 Particulier</button>
                  <button type="button" onClick={() => setForm({...form, locataire_type: 'morale'})} style={{ flex: 1, padding: 10, borderRadius: 10, border: `2px solid ${form.locataire_type === 'morale' ? '#7c3aed' : '#e5e7eb'}`, background: form.locataire_type === 'morale' ? '#f5f3ff' : 'white', color: form.locataire_type === 'morale' ? '#7c3aed' : '#6b7280', fontWeight: 600, cursor: 'pointer', fontSize: 14 }}>🏢 Société</button>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
                ...
                <button onClick={() => { if (form.locataire_type === 'morale') { if (!form.locataire_denomination || !form.locataire_email) { alert('Dénomination sociale et email obligatoires.'); return } } else { if (!form.locataire_prenom || !form.locataire_nom || !form.locataire_email) { alert('Prénom, nom et email obligatoires.'); return } } setEtape(3) }}
                  style={{ flex: 2, background: '#7c3aed', color: 'white', padding: 12, borderRadius: 10, border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: 15 }}>
                  Suivant → Bien & loyer
                </button>
              </div>
            </div>
          ...
          {etape === 3 && (
            <div>
              ...
              {lotsDisponibles.length > 0 && (
                <div style={{ marginTop: 12, marginBottom: 14, background: '#f5f3ff', border: '1px solid #ddd6fe', borderRadius: 10, padding: 14 }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: '#7c3aed', marginBottom: 10 }}>🛋️ Sélectionnez les lots concernés par ce bail</p>
                  ...
                  {lotsSelectionnes.length > 0 && (
                    <div style={{ background: 'white', borderRadius: 8, padding: 10, marginTop: 8, border: '1px solid #ddd6fe' }}>
              ...
              <div style={{ marginBottom: 14 }}><label style={lbl}>Équipements inclus *</label><input style={inp} value={form.equipements} onChange={e => setForm({...form, equipements: e.target.value})} placeholder="Lit, canapé, réfrigérateur, four, table, chaises..." /></div>
              {/* DIFFÉRENCE MEUBLÉ : avertissement dépôt 2 mois max */}
              <div style={{ background: '#f5f3ff', border: '1px solid #ddd6fe', borderRadius: 10, padding: 10, marginBottom: 14 }}>
                <p style={{ fontSize: 12, color: '#7c3aed', margin: 0 }}>ℹ️ Pour un bail meublé, le dépôt de garantie ne peut pas dépasser <strong>2 mois</strong> de loyer hors charges.</p>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 14 }}>
                ...
                <div><label style={lbl}>Dépôt garantie (€)</label><input style={inp} type="number" value={form.depot_garantie} onChange={e => setForm({...form, depot_garantie: e.target.value})} placeholder="Max 2 mois HC" /></div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
                <div>
              ...
              {form.loyer_hc && (
                <div style={{ background: '#f5f3ff', borderRadius: 10, padding: 14, marginBottom: 16, border: '1px solid #ddd6fe' }}>
                  <p style={{ fontSize: 13, color: '#7c3aed', margin: 0 }}>
                    Total CC : <strong>{(parseFloat(form.loyer_hc) + parseFloat(form.charges || 0)).toFixed(2)}€</strong> — Dépôt max : <strong>{(parseFloat(form.loyer_hc) * 2).toFixed(2)}€</strong>
                  </p>
                </div>
              )}
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 11, color: '#9ca3af', marginTop: 4 }}>Vide = reconduction tacite 1 an</p>
                </div>
              </div>
              {plan === 'automatique' && (
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <textarea style={{ ...inp, minHeight: 100, resize: 'vertical' }} value={form.clauses} onChange={e => setForm({...form, clauses: e.target.value})} placeholder="Ex : inventaire annexé, conditions spécifiques..." />
              </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24 }}>
                ...
                <div style={{ background: '#f9fafb', borderRadius: 14, padding: '18px 20px', display: 'flex', gap: 16, alignItems: 'center', border: '2px solid transparent', opacity: 0.5, cursor: 'not-allowed' }}>
                  ...
                  <div><h3 style={{ fontSize: 15, fontWeight: 700, color: '#6b7280', margin: '0 0 3px' }}>Signature électronique via Yousign</h3><p style={{ fontSize: 13, color: '#9ca3af', margin: 0 }}>Temporairement indisponible.</p></div>
                </div>
                <div onClick={envoyerPourSignatureLocataire}
                  ...
                  onMouseLeave={e => e.currentTarget.style.border = '2px solid transparent'}>
                  <div style={{ fontSize: 32 }}>📧</div>
                  <div><h3 style={{ fontSize: 15, fontWeight: 700, color: '#2563eb', margin: '0 0 3px' }}>Envoyer par email pour signature</h3><p style={{ fontSize: 13, color: '#6b7280', margin: 0 }}>Vous signez immédiatement — le locataire signe à distance, le bail passe en actif dès sa signature.</p></div>
            ...
            <div>
                ...
                {!signatureBailleur || signatureActive === 'bailleur' ? (
                  <>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, cursor: 'pointer', fontSize: 13, color: '#374151' }}>
                      <input type="checkbox" checked={luApprouveBailleur} onChange={e => setLuApprouveBailleur(e.target.checked)} style={{ width: 16, height: 16 }} />
                      Je certifie avoir <strong>&nbsp;lu et approuvé&nbsp;</strong> le contenu du bail
                    </label>
                    <canvas ref={signatureActive === 'bailleur' || !signatureActive ? canvasRef : null}
                      ...
                      style={{ border: `2px dashed ${signatureActive === 'bailleur' ? '#7c3aed' : '#d1d5db'}`, borderRadius: 10, width: '100%', height: 140, cursor: 'crosshair', touchAction: 'none', background: '#fafafa', display: 'block' }} />
                    ...
                    {!signatureActive && !signatureBailleur && (
                      <button onClick={() => setSignatureActive('bailleur')} style={{ marginTop: 8, background: '#f5f3ff', color: '#7c3aed', border: '1px solid #ddd6fe', borderRadius: 8, padding: '8px 16px', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>🖊️ Signer ici</button>
                    )}
                  </>
                ) : (
                  <div>
              ...
              <div style={{ background: '#f9fafb', borderRadius: 12, padding: 16, marginBottom: 20, border: '1px solid #e5e7eb' }}>
                ...
                {!signatureLocataire || signatureActive === 'locataire' ? (
                  <>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, cursor: 'pointer', fontSize: 13, color: '#374151' }}>
                      <input type="checkbox" checked={luApprouveLocataire} onChange={e => setLuApprouveLocataire(e.target.checked)} style={{ width: 16, height: 16 }} />
                      Je certifie avoir <strong>&nbsp;lu et approuvé&nbsp;</strong> le contenu du bail
                    </label>
                    <canvas ref={signatureActive === 'locataire' ? canvasRef : null}
                      ...
                      onTouchMove={draw} onTouchEnd={stopDraw}
                      width={520} height={140}
                      style={{ border: `2px dashed ${signatureActive === 'locataire' ? '#7c3aed' : '#d1d5db'}`, borderRadius: 10, width: '100%', height: 140, cursor: 'crosshair', touchAction: 'none', background: '#fafafa', display: 'block' }} />
                    ...
                    {!signatureActive && !signatureLocataire && (
                      <button onClick={() => setSignatureActive('locataire')} style={{ marginTop: 8, background: '#f5f3ff', color: '#7c3aed', border: '1px solid #ddd6fe', borderRadius: 8, padding: '8px 16px', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>🖊️ Signer ici</button>
                    )}
                  </>
                ) : (
                  <div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={finaliserEtSauvegarder} disabled={loading || !signatureBailleur || !signatureLocataire || !luApprouveBailleur || !luApprouveLocataire}
