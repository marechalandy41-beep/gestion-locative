...
export default function NouveauBailNonMeuble() {
  ...
  const [form, setForm] = useState({
    ...
    bailleur_representant_type: 'physique', bailleur_representant_denomination: '', bailleur_representant_personne: '',
    bailleur_nom: '', bailleur_prenom: '', bailleur_adresse: '',
    ...
    locataire_representant_type: 'physique', locataire_representant_denomination: '', locataire_representant_personne: '',
    locataire_nom: '', locataire_prenom: '', locataire_email: '',
...
async function envoyerVersYousign() {
  ...
  async function finaliserEtSauvegarder() {
    ...
    try {
      ...
      // Mention "Lu et approuvé" en haut à l'intérieur de chaque cadre
      ...
      doc.text('Document généré par Ma Gestion-Locative — Conforme loi n°89-462 du 6 juillet 1989 et loi ALUR du 24 mars 2014', pageW / 2, y, { align: 'center' })
        ...
        <div style={{ background: 'white', borderRadius: 16, padding: 28, boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
            ...
            <div>
              ...
              <div style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button type="button" onClick={() => setForm({...form, bailleur_type: 'particulier'})}
                    style={{ flex: 1, padding: 10, borderRadius: 10, border: `2px solid ${form.bailleur_type === 'particulier' ? '#2563eb' : '#e5e7eb'}`, background: form.bailleur_type === 'particulier' ? '#eff6ff' : 'white', color: form.bailleur_type === 'particulier' ? '#2563eb' : '#6b7280', fontWeight: 600, cursor: 'pointer', fontSize: 14 }}>
                    👤 Particulier
                  </button>
                  <button type="button" onClick={() => setForm({...form, bailleur_type: 'morale'})}
                    style={{ flex: 1, padding: 10, borderRadius: 10, border: `2px solid ${form.bailleur_type === 'morale' ? '#2563eb' : '#e5e7eb'}`, background: form.bailleur_type === 'morale' ? '#eff6ff' : 'white', color: form.bailleur_type === 'morale' ? '#2563eb' : '#6b7280', fontWeight: 600, cursor: 'pointer', fontSize: 14 }}>
                    🏢 Société (SCI, SARL...)
                  </button>
                </div>
              </div>
              {form.bailleur_type === 'morale' && (
                <div style={{ background: '#f9fafb', borderRadius: 10, padding: 14, marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                    ...
                    <div>
                      <label style={lbl}>Représentée par</label>
                      <select style={inp} value={form.bailleur_representant_type} onChange={e => setForm({...form, bailleur_representant_type: e.target.value})}>
                        <option value="physique">Une personne physique</option>
                        <option value="morale">Une autre société</option>
                      </select>
                    </div>
                  </div>
                  {form.bailleur_representant_type === 'physique' && (
                    <div style={{ marginTop: 12 }}>
                      <label style={lbl}>Nom du représentant légal</label>
                      <input style={inp} value={form.bailleur_representant} onChange={e => setForm({...form, bailleur_representant: e.target.value})} placeholder="Jean Dupont, gérant" />
                    </div>
                  )}
                  {form.bailleur_representant_type === 'morale' && (
                    <div style={{ marginTop: 12, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                      <div>
                        <label style={lbl}>Société représentante</label>
                        <input style={inp} value={form.bailleur_representant_denomination} onChange={e => setForm({...form, bailleur_representant_denomination: e.target.value})} placeholder="Holding Martin SAS" />
                      </div>
                      <div>
                        <label style={lbl}>Elle-même représentée par</label>
                        <input style={inp} value={form.bailleur_representant_personne} onChange={e => setForm({...form, bailleur_representant_personne: e.target.value})} placeholder="Jean Dupont, gérant" />
                      </div>
                    </div>
                  )}
                </div>
              ...
              <button onClick={() => {
                  if (form.bailleur_type === 'morale') {
                    if (!form.bailleur_denomination || !form.bailleur_adresse) { alert('Dénomination sociale et adresse obligatoires.'); return }
                  } else {
                    if (!form.bailleur_prenom || !form.bailleur_nom || !form.bailleur_adresse) { alert('Prénom, nom et adresse obligatoires.'); return }
                  }
                  setEtape(2)
                }}
          ...
          {etape === 2 && (
            <div>
              ...
              <div style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button type="button" onClick={() => setForm({...form, locataire_type: 'particulier'})}
                    style={{ flex: 1, padding: 10, borderRadius: 10, border: `2px solid ${form.locataire_type === 'particulier' ? '#2563eb' : '#e5e7eb'}`, background: form.locataire_type === 'particulier' ? '#eff6ff' : 'white', color: form.locataire_type === 'particulier' ? '#2563eb' : '#6b7280', fontWeight: 600, cursor: 'pointer', fontSize: 14 }}>
                    👤 Particulier
                  </button>
                  <button type="button" onClick={() => setForm({...form, locataire_type: 'morale'})}
                    style={{ flex: 1, padding: 10, borderRadius: 10, border: `2px solid ${form.locataire_type === 'morale' ? '#2563eb' : '#e5e7eb'}`, background: form.locataire_type === 'morale' ? '#eff6ff' : 'white', color: form.locataire_type === 'morale' ? '#2563eb' : '#6b7280', fontWeight: 600, cursor: 'pointer', fontSize: 14 }}>
                    🏢 Société
                  </button>
                </div>
              </div>
              {form.locataire_type === 'morale' && (
                <div style={{ background: '#f9fafb', borderRadius: 10, padding: 14, marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                    ...
                    <div>
                      <label style={lbl}>Représentée par</label>
                      <select style={inp} value={form.locataire_representant_type} onChange={e => setForm({...form, locataire_representant_type: e.target.value})}>
                        <option value="physique">Une personne physique</option>
                        <option value="morale">Une autre société</option>
                      </select>
                    </div>
                  ...
                  {form.locataire_representant_type === 'physique' && (
                    <div style={{ marginTop: 12 }}>
                      <label style={lbl}>Nom du représentant légal</label>
                      <input style={inp} value={form.locataire_representant} onChange={e => setForm({...form, locataire_representant: e.target.value})} placeholder="Jean Dupont, gérant" />
                    </div>
                  )}
                  {form.locataire_representant_type === 'morale' && (
                    <div style={{ marginTop: 12, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                      ...
                      <div><label style={lbl}>Elle-même représentée par</label><input style={inp} value={form.locataire_representant_personne} onChange={e => setForm({...form, locataire_representant_personne: e.target.value})} placeholder="Jean Dupont, gérant" /></div>
                    </div>
                  )}
                </div>
              )}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24 }}>
      ...
      <div onClick={async () => {
        ...
        const { data: bail, error } = await supabase.from('Baux').insert({
          ...
          bailleur_lieu_naissance: form.bailleur_lieu_naissance, bailleur_nationalite: form.bailleur_nationalite,
          locataire_prenom: form.locataire_prenom, locataire_nom: form.locataire_nom,
          locataire_email: form.locataire_email,
      locataire_telephone: form.locataire_telephone,
      locataire_naissance: form.locataire_naissance || null,
      locataire_nationalite: form.locataire_nationalite,
      locataire_profession: form.locataire_profession,
      locataire_adresse: form.locataire_adresse,
      bailleur_type: form.bailleur_type,
      bailleur_denomination: form.bailleur_denomination,
      bailleur_forme_juridique: form.bailleur_forme_juridique,
      bailleur_siren: form.bailleur_siren,
      bailleur_representant: form.bailleur_representant,
      bailleur_representant_type: form.bailleur_representant_type,
      bailleur_representant_denomination: form.bailleur_representant_denomination,
      bailleur_representant_personne: form.bailleur_representant_personne,
      locataire_type: form.locataire_type,
      locataire_denomination: form.locataire_denomination,
      locataire_forme_juridique: form.locataire_forme_juridique,
      locataire_siren: form.locataire_siren,
      locataire_representant: form.locataire_representant,
      locataire_representant_type: form.locataire_representant_type,
      locataire_representant_denomination: form.locataire_representant_denomination,
      locataire_representant_personne: form.locataire_representant_personne,
          surface_habitable: parseFloat(form.surface_habitable) || null,
          nombre_pieces: parseInt(form.nombre_pieces) || null,
          etage: form.etage, equipements: form.equipements,
