...
export default function Biens() {
  ...
  // ========== FONCTION: CHARGER LES BIENS DE L'UTILISATEUR ==========
  async function chargerBiens(userId) {
    setLoading(true);
    const { data, error } = await supabase
      .from('Biens')
      .select('*')
      .eq('user_id', userId); // Filtre par utilisateur connecté
  ...
  async function ajouterBien() {
    ...
    const { error } = await supabase.from('Biens').insert([{
      ...
      user_id: user.id // Lie le bien à l'utilisateur connecté
    }]);
    if (!error) {
      setShowForm(false);
      setNewBien(formVide);
      setEtapeForm(1);
      chargerBiens(user.id);
    } else {
      ...
      <div style={{maxWidth:1280, margin:'0 auto', padding:'32px 24px'}}>
          ...
          <div style={{background:'white', borderRadius:16, border:'1px solid #e5e7eb', padding:32, marginBottom:32, boxShadow:'0 1px 3px rgba(0,0,0,0.05)'}}>
            ...
            {etapeForm === 1 && (
              <div>
                ...
                <div style={{display:'flex', gap:8}}>
                  <button onClick={() => { if (newBien.nom && newBien.rue && newBien.code_postal && newBien.ville) setEtapeForm(2); }} style={{background:'#2563eb', color:'white', padding:'10px 24px', borderRadius:8, border:'none', cursor:'pointer', fontWeight:600, fontSize:14}}>
                    Suivant →
                  </button>
                  <button onClick={() => { setShowForm(false); setNewBien(formVide); }} style={{background:'#f3f4f6', color:'#374151', padding:'10px 24px', borderRadius:8, border:'none', cursor:'pointer', fontWeight:600, fontSize:14}}>
                    Annuler
                  </button>
                </div>
              </div>
            ...
            {etapeForm === 2 && (
              <div>
                ...
                <div style={{display:'flex', gap:8}}>
                  <button onClick={ajouterBien} style={{background:'#2563eb', color:'white', padding:'10px 24px', borderRadius:8, border:'none', cursor:'pointer', fontWeight:600, fontSize:14}}>
                    ✅ Enregistrer le bien
                  </button>
                  <button onClick={() => setEtapeForm(1)} style={{background:'#f3f4f6', color:'#374151', padding:'10px 24px', borderRadius:8, border:'none', cursor:'pointer', fontWeight:600, fontSize:14}}>
                    ← Retour
                  </button>
                </div>
              </div>
        ...
        {loading ? (
          ...
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:24}}>
            {biens.map(bien => (
              <div key={bien.id} onClick={() => handleSelect(bien)} style={{background:'white', borderRadius:20, border: selectionne === bien.id ? '2px solid #2563eb' : '1px solid #f3f4f6', boxShadow:'0 1px 3px rgba(0,0,0,0.05)', padding:24, cursor:'pointer'}}>
